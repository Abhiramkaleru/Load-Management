...
router.get("/prepare", authenticate, async (req, res) => {
  try {
    ...
    console.log("load_request rows:", result.load_request);
    console.log("employee rows:", result.employee);
    res.json(result);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Sync prepare failed" });
...
router.post("/upload", authenticate, async (req, res) => {
  ...
  try {
    ...
    for (const request of requests) {
      ...
      if (loadResult.rows.length === 0) {
        throw new Error(
          "Load request uid belongs to another employee or does not exist",
        );
      }
...
router.get("/changes", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (requestIds.length > 0) {
      const lineResult = await pool.query(
        `SELECT id, load_request_id, sku_id, quantity, uom, approved_qty 
         FROM load_request_line 
         WHERE load_request_id = ANY($1::uuid[])`,
        [requestIds],
      );
